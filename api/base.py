...
class Chaoxing:
    ...
    def login(self, login_with_cookies=False):
        if login_with_cookies:
            ...
            logger.debug("Loaded cookies for login")
    ...
    def study_video(self, _course, _job, _job_info, _speed: float = 1.0, _type: Literal["Video", "Audio"] = "Video") -> StudyResult:
        ...
        task_name = _job.get("name") or _job.get("jobid") or "video"

        def emit_progress(status: str, current_time: float) -> None:
                ...
                self.progress_callback(
                    {
                        ...
                        "objectid": _job.get("objectid"),
                        "course_id": _course.get("courseId"),
                        "course_title": _course.get("title", ""),
                        "play_time": int(current_time),
                        "duration": int(duration),
                        "percent": int(min(100, max(0, (float(current_time) / duration) * 100))) if duration else 0,
                    }
                )
        ...
        emit_progress("playing", play_time)
        ...
        if passed:
            ...
            emit_progress("completed", duration)
    ...
    def study_work(self, _course, _job, _job_info) -> StudyResult:
        ...
        def emit_work_progress(status: str, question: dict | None = None, index: int = 0, total: int = 0) -> None:
            if not callable(self.progress_callback):
                return
            qid = str((question or {}).get("id") or index or "work")
            title = str((question or {}).get("title") or _job.get("name") or _job.get("jobid") or "work")
            try:
                self.progress_callback(
                    {
                        "status": status,
                        "type": "work",
                        "task_name": title,
                        "jobid": f"{_job.get('jobid', 'work')}:{qid}",
                        "parent_jobid": _job.get("jobid"),
                        "course_id": _course.get("courseId"),
                        "course_title": _course.get("title", ""),
                        "module_id": _job_info.get("knowledgeid"),
                        "module_title": _job.get("name") or "章节测验",
                        "play_time": index,
                        "duration": total,
                        "percent": int(min(100, max(0, (float(index) / total) * 100))) if total else 0,
                        "ephemeral": True,
                    }
                )
            except Exception as exc:
                logger.debug("work progress callback failed: {}", exc)
        ...
        found_answers = 0
        for question_index, q in enumerate(questions["questions"], start=1):
            emit_work_progress("answering", q, question_index, total_questions)
            ...
            emit_work_progress("completed", q, question_index, total_questions)
