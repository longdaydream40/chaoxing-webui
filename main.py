...
def init_chaoxing(common_config, tiku_config):
    ...
    if provider in ["AI", "SiliconFlow"]:
        ...
        if check_connection:
            logger.info("正在验证大模型配置 (provider={})...", provider)
            if not tiku.check_llm_connection():
                logger.error("大模型连接检查失败")
                if sys.stdin.isatty():
                    try:
                        ...
                        logger.warning("标准输入不可用，大模型连接检查失败后自动继续运行")
                        choice = ""
                    if choice not in ("", "y", "yes"):
                        raise RuntimeError("用户取消运行")
                    logger.info("用户选择继续运行...")
                else:
                    logger.warning("当前为非交互环境，大模型连接检查失败后自动继续运行")
...
class JobProcessor:
    ...
    def run(self):
        ...
        for _ in range(self.worker_num):
            self.task_queue.put(ChapterTask(index=self.SENTINEL_INDEX, point={}))
    ...
    def worker_thread(self):
        ...
        while True:
            ...
            if task.index == self.SENTINEL_INDEX or self._shutdown:
